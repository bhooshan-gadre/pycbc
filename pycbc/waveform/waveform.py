...
cuda_td = dict(
    list(_lalsim_td_approximants.items()) +
    list(_cuda_td_approximants.items()))
cuda_fd = dict(
    list(_lalsim_fd_approximants.items()) +
    list(_cuda_fd_approximants.items()))
        ...
        if isinstance(obj, numpy.core.records.record):
            ...
            for name in dir(obj):
                try:
                    ...
                    if not name.startswith('__') and not inspect.ismethod(
                            value):
                        pr[name] = value
                except:
                    continue
...
def _base_get_td_waveform_from_fd(template=None, rwrap=0.2, **params):
    ...
    if nparams['approximant'] not in fd_det:
        ...
        hp = wfutils.fd_to_td(hp,
                              delta_t=params['delta_t'],
                              left_window=(nparams['f_lower'],
                                           params['f_lower']))
        hc = wfutils.fd_to_td(hc,
                              delta_t=params['delta_t'],
                              left_window=(nparams['f_lower'],
                                           params['f_lower']))
        return hp, hc
        ...
        for ifo in wfs.keys():
            ...
            wfs[ifo] = wfutils.fd_to_td(wfs[ifo],
...
def get_interpolated_fd_waveform(dtype=numpy.complex64,
                                 return_hc=True,
                                 **params):
    ...
    hp = interpolate_complex_frequency(hp,
                                       df,
                                       zeros_offset=offset,
                                       side='left')
    if hc is not None:
        hc = interpolate_complex_frequency(hc,
                                           df,
                                           zeros_offset=offset,
                                           side='left')
...
def get_imr_length(approx, **kwds):
    ...
    return pnutils.get_imr_duration(m1,
                                    m2,
                                    s1z,
                                    s2z,
                                    f_low,
...
_filter_time_lengths["SEOBNRv4E"] = seobnrv4_length_in_time
_filter_time_lengths["SEOBNRv4E_opt"] = seobnrv4_length_in_time
_filter_time_lengths["SEOBNRv4E_opt1"] = seobnrv4_length_in_time
_filter_time_lengths["SEOBNRv4EHM"] = seobnrv4_length_in_time
_filter_time_lengths["SEOBNRv4EHM_opt"] = seobnrv4_length_in_time
_filter_time_lengths["SEOBNRv4EHM_opt1"] = seobnrv4_length_in_time
...
__all__ = [
    "get_td_waveform", "get_td_det_waveform_from_fd_det", "get_fd_waveform",
    "get_fd_waveform_sequence", "get_fd_det_waveform",
    "get_fd_det_waveform_sequence", "get_fd_waveform_from_td",
    "print_td_approximants", "print_fd_approximants", "td_approximants",
    "fd_approximants", "get_waveform_filter", "filter_approximants",
